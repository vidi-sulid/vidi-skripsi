    ...
    <form id='form1'>
            ...
            <div class="row">
                <div class="col mb-3">
                    ...
                    <input type="text" id="code" name="code" maxlength="8" class="form-control"
                        placeholder="Kode" value="{{ $productLoan->code }}" readonly disabled />
                </div>
                <div class="col-6 mb-3">
                    <label for="nameBasic" class="form-label">Nama</label>
                    <input type="text" id="name" name="name" class="form-control"
                        value="{{ $productLoan->name }}" placeholder="Nama" />
                </div>
            ...
            <div class="row">
                    ...
                    <select id="account_loan" name="account_loan" class="select2 form-select"
                        ...
                        @foreach (App\Models\Master\Coa::where('type', 0)->get() as $rekening)
                            <option value="{{ $rekening->code }}"
                                @if ($productLoan->account_loan == $rekening->code) {{ 'selected' }} @endif>
                ...
                <div class="col-6">
                    ...
                    <select id="account_income_administration" name="account_income_administration"
                        ...
                        @foreach (App\Models\Master\Coa::where('type', 0)->get() as $rekening)
                            <option value="{{ $rekening->code }}"
                                @if ($productLoan->account_income_administration == $rekening->code) {{ 'selected' }} @endif>
                                {{ $rekening->code . '-' . $rekening->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="row">
                <div class="col-6">
                    ...
                    <select id="account_income_interest" name="account_income_interest" class="select2 form-select"
                        ...
                        @foreach (App\Models\Master\Coa::where('type', 0)->get() as $rekening)
                            <option value="{{ $rekening->code }}"
                                @if ($productLoan->account_income_interest == $rekening->code) {{ 'selected' }} @endif>
                                {{ $rekening->code . '-' . $rekening->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-6">
                    ...
                    <select id="account_dutystamp" name="account_dutystamp" class="select2 form-select"
                        ...
                        @foreach (App\Models\Master\Coa::where('type', 0)->get() as $rekening)
                            <option value="{{ $rekening->code }}"
                                @if ($productLoan->account_dutystamp == $rekening->code) {{ 'selected' }} @endif>
                                {{ $rekening->code . '-' . $rekening->name }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="modal-footer">
            ...
            <button type="button" p class="btn btn-primary"
                onclick="save('{{ route('product-loan.update', $productLoan->id) }}','put')"><i class="bx bx-save"></i>
                Save
                changes</button>
        </div>
    </form>
